...
\begin{enumerate}
  \item Comenzamos restaurando la instantánea de la instalación de Ubuntu con red.
  \item Vamos a los ajustes de almacenamiento, segundo disco y marcamos la opción de \textit{conectable en caliente}.
  \item Arrancamos la máquina y eliminamos el disco accediento a los ajustes de almacenamiento.
  \item Vemos que Ubuntu nos avisa de que se ha perdido la conexión al disco. Si ejecutamos \textit{lsblk} veremos que sólo hay un disco. Hemos comprobado que el sistema continúa funcional tras eliminar uno de los discos (como cabía esperar).
  \item Ahora eliminaremos un disco en frío. Comenzamos restaurando la instantánea de red y eliminando el disco.
  \item Arrancamos la máquina y vemos que no arranca, se queda bloqueada al intentar leer los volúmenes físicos.
  \item Tendremos que esperar 300 segundos para que cargue \textit{initramfs}, un shell mínimo que permite realizar labores de mantenimiento.
  \item Podemos ver información de nuestro sistema RAID mediante
  \begin{lstlisting}
    (initramfs)> cat /proc/mdstat
  \end{lstlisting}
  \item Podemos ver que RAID está inactivo. El campo \textit{Personalities} nos dice los tipos de RAID que acepta nuestra máquina.
  \item También podemos ver el diario del sistema (\textit{journalctl}) mediante
  \begin{lstlisting}
    (initramfs)> dmesg
  \end{lstlisting}
  \item Deducimos que debemos activar RAID. Para ello ejecutamos el siguiente comando (extraído del manual):
  \begin{lstlisting}
    (initramfs)> mdadm --run /dev/md0
  \end{lstlisting}
  \item Podemos ver mediante
  \begin{lstlisting}
    (initramfs)> cat /proc/mdstat
  \end{lstlisting}
  que el RAID ya está activo.
  \item Pulsamos Ctrl+D para arrancar el sistema y vemos que podemos iniciar sesión.
  \item Ahora toca añadir un nuevo disco al RAID. Comenzamos añadiéndolo en las opciones de almacenamiento.
  \item Comprobamos mediante \textit{lsblk} que se ha añadido el nuevo disco a la máquina.
  \item Ahora lo añadimos al RAID:
  \begin{lstlisting}
    $> sudo mdadm --add /dev/md0 /dev/sdb
  \end{lstlisting}
  \item Podemos ver el proceso de clonado de los discos mediante:
  \begin{lstlisting}
    $> watch cat /proc/mdadm
  \end{lstlisting}
  \item Por último, cuando termine el proceso podemos comprobar que todo está configurado correctamente mediante \textit{lsblk}.
\end{enumerate}
...
\section{Práctica 4: Benchmarking y Ajuste del Sistema}

\subsection{Sesión 1}

En esta sesión instalaremos la suite de benchmarking \textit{phoronix} y ejecutaremos dos tests sobre Ubuntu y CentOS para comparar su rendimiento. También ejecutaremos \textit{ab} (Apache Benchmark) desde el host para evaluar el rendimiento de las máquinas al responder peticiones HTTP.

\begin{enumerate}
  \item Comenzamos instalando la suite en las máquinas:
  \begin{lstlisting}
    CentOS #> yum install -y phoronix-test-suite
    Ubuntu $> sudo apt install -y phoronix-test-suite php-zip
  \end{lstlisting}
  \item Podemos ver los tests disponibles mediante:
  \begin{lstlisting}
    $> phoronix-test-suite list-available-tests
  \end{lstlisting}
  \item Elegimos dos y los ejecutamos mediante:
  \begin{lstlisting}
  ...
  \end{lstlisting}
  \item Se nos pedirá nuestra clave para instalar las dependencias. La introducimos y pulsamos Enter. No se mostrará ninguna salida hasta que no se instalen todas.
  \item Elegimos las opciones deseadas del test y le damos nombre, ID y descripción.
  \item El test comenzará a ejecutarse. Cuando termine, podremos ver los resultados. En mi caso, Ubuntu saca mejor puntuación en ambos.
\end{enumerate}
Pasemos ahora a Apache Benchmark:
\begin{enumerate}
  \item Comenzamos instalando \textit{apache2} en nuestro host:
  \begin{lstlisting}
    Host $> sudo apt install -y apache2
  \end{lstlisting}
  \item La sintaxis de la herramienta es la siguiente:
  \begin{lstlisting}
  ...
  \item Cuando termine, podremos ver varias métricas. La que nos interesa es el tiempo medio de respuesta, \textit{Time per request (mean)}. En mi caso, Ubuntu es más rápido que CentOS.
\end{enumerate}

También podemos ejecutar los tests desde un contenedor de Docker:
\begin{enumerate}
  \item Comenzamos instalando Docker:
  \begin{lstlisting}
  ...
  \end{lstlisting}
  \item Cerramos sesión y volvemos a loguearnos.
  \item Descargamos y arrancamos el contenedor de phoronix:
  \begin{lstlisting}
    $> docker run phoronix/pts
  \end{lstlisting}
  \item Ejecutamos los tests:
\end{enumerate}
...
\begin{enumerate}
  \item Comenzamos descargando la utilidad desde \href{http://apache.rediris.es//jmeter/binaries/apache-jmeter-5.2.1.zip}{aquí} en nuestro host. La descomprimimos y la abrimos:
  \begin{lstlisting}
    Host $> java -jar <path de la app>/bin/ApacheJMeter.jar
  \end{lstlisting}
  \item Instalamos Docker en Ubuntu.
  \item Descargamos la aplicación en Ubuntu:
  \begin{lstlisting}
    Ubuntu $> git clone https://github.com/davidPalomar-ugr/iseP4JMeter.git
  \end{lstlisting}
  \item Nos situamos en la carpeta del proyecto y levantamos los contenedores:
  \begin{lstlisting}
    Ubuntu $> cd iseP4JMeter
    Ubuntu $> docker-compose up -d
  \end{lstlisting}
  \item Comprobamos que la app funciona ejecutando el script \textit{pruebaEntorno.sh}.
  \begin{lstlisting}
    Ubuntu $> sh pruebaEntorno.sh
  \end{lstlisting}
  \item Comenzamos a crear el plan de pruebas en JMeter:
  \item Establecemos las variables HOST y PORT en el plan de pruebas:
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/planpruebas}
  \end{figure}
  \item Creamos unos valores por defecto para la petición HTTP: botón derecho en Plan de Pruebas, Añadir, Elemento de Configuración, Valores por defecto para petición HTTP. Configuramos el host y el puerto referenciando a las variables ya definidas.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/httpdefault}
  \end{figure}
  \item Creamos la autenticación para la API: Plan de Pruebas, Añadir, Elemento de Configuración, Gestor de Autorización HTTP. Configuramos:
  \begin{itemize}
    \item URL base: http://\$\{HOST\}:\$\{PORT\}/api/v1/auth/login
    \item Nombre de usuario: etsiiApi
    \item Contraseña: laApiDeLaETSIIDaLache
  \end{itemize}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/basicauth}
  \end{figure}
  \item Creamos el grupo de hilos de Alumnos: Plan de Pruebas, Añadir, Hilos (usuarios), Grupo de Hilos. Establecemos dos hilos.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/alumnos}
  \end{figure}
  \item Añadimos el acceso a los credenciales de alumnos almacenados en el archivo CSV. Alumnos, Añadir, Elemento de Configuración, Configuración del CSV Data Set. Establecemos la ruta del archivo CSV.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/credencialesalumnos}
  \end{figure}
  \item Creamos la petición de login del alumno. Alumnos, Añadir, Muestreador, Petición HTTP. Configuramos los siguientes campos:
  \begin{itemize}
    \item Método: POST
    \item Ruta: /api/v1/auth/login
    \item Parámetros para la petición.
  \end{itemize}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/loginalumno}
  \end{figure}
  \item Ahora debemos extraer el token JWT. Login Alumno, Añadir, Post Procesadores, Extractor de expresiones regulares. Configuramos los siguientes campos:
  \begin{itemize}
    \item Nombre de referencia: token
    \item Expresión regular: .+
    \item Plantilla: \$0\$
  \end{itemize}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/extractjwt}
  \end{figure}
  \item Esperamos a la extracción del token: Alumnos, Añadir, Temporizador, Temporizador Aleatorio Gaussiano.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/temporizador}
  \end{figure}
  \item Añadimos la petición para recuperar los datos del alumno. Alumnos, Añadir, Muestreador, Petición HTTP. Configuramos los siguientes campos:
  \begin{itemize}
    \item Método: GET
    \item Ruta: /api/v1/alumnos/alumno/\$\{\_\_urlencode(\$\{login\})\} . La función \textit{\_urlencode} se utiliza para traducir caracteres como la \textit{@} a un formato aceptado por HTTP.
    \item Parámetro \textit{login} para la petición.
  \end{itemize}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/recuperardatosalumno}
  \end{figure}
  \item Añadimos un gestor de cabecera para el token: Alumnos, Añadir, Elemento de Configuración, Gestor de Cabecera HTTP.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/httpheader}
  \end{figure}
  \item Creamos ahora el hilo de administradores igual que el de alumnos.
  \item Añadimos la configuración del dataset con los credenciales de administrador seleccionando el CSV correspondiente.
  \item Creamos la petición de login para el administrador, la extracción del token JWT y la espera igual que en alumnos.
  \item Creamos la petición para el logueo del acceso de los administradores. Administradores, Añadir, Muestreador, Muestreador de Acceso a Log. Establecemos los siguientes campos:
  \begin{itemize}
    \item Protocolo: http
    \item Servidor: \$\{HOST\}
    \item Puerto: \$\{PORT \}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jmeter/accesoadministradores}
  \end{figure}
  \item Añadimos el gestor de cabecera HTTP al acceso de administradores (igual que en alumnos).
  \item Añadimos un temporizador aleatorio gaussiano al grupo de hilos de administradores.
  \item Añadimos los gráficos. Plan de Pruebas, Añadir, Receptor:
  \begin{itemize}
    \item Reporte resumen.
    \item Ver resultados en árbol.
    \item Informe agregado.
    \item Árbol de resultados.
  \end{itemize}
  \item Damos al icono de Play y tras 10 segundos a STOP. En el reporte resumen tendremos que tener todos los errores al 0\%.
\end{enumerate}

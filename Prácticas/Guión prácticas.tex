...
\end{figure}
\newpage
La estrictura a crear es la siguiente:
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[arrow/.style = {thick,-stealth}, node distance=2cm]
		\tikzset{set/.style={draw,rectangle,inner sep=0pt,align=center}}
		\node[fit={(0,0) (2,1)}, inner sep=0pt, draw=black, thick, text centered] (sda) {sda};
		\node[fit={(3,0) (5,1)}, inner sep=0pt, draw=black, thick, text centered] (sdb) {sdb};

		\node[fit={(0,2) (2,3)}, inner sep=0pt, draw=black, thick, text centered] (sdapv) {sda};
		\node[fit={(3,2) (5,3)}, inner sep=0pt, draw=black, thick, text centered] (sdbpv) {sdb};

	\node[circle,above right of=sdapv, inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm, distance = 1cm] (cl) {cl};

	\node[circle,above left of=cl, inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm, distance = 1cm] (/) {/};

	\node[circle,above right of=cl, inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm, distance = 1cm] (swap) {swap};
		...
		\draw [decorate,decoration={brace,amplitude=10pt, raise=2.5pt, mirror}]	(6,3.35) -- (6,4.35) node [black,midway,xshift=1cm] {\footnotesize VG};
		\draw [decorate,decoration={brace,amplitude=10pt, raise=2.5pt, mirror}]	(6,4.7) -- (6,5.7) node [black,midway,xshift=1cm] {\footnotesize LV};

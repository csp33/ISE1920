...
\begin{figure}[H]
	...
	\begin{tikzpicture}[arrow/.style = {thick,-stealth}, node distance=2cm]
		...
		\node[ellipse,above of=CENTER, inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm,fill=yellow!20] (cl) {cl};
		...
		\node[diamond, above of=raizlv,inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm,fill=red!20] (raizfs) {raíz};

		\node[diamond, above of=arranquelv,inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm,fill=red!20] (arranquefs) {arranque};

		\node[diamond, above of=hogarlv,inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm,fill=red!20] (hogarfs) {hogar};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[arrow/.style = {thick,-stealth}, node distance=2cm]
		...
		% LVs
		\node[ellipse,above left of=cl, inner sep=0pt, draw=black, thick, text centered, minimum size = 1cm,fill=brown!25] (/lv) {/};
...
\begin{enumerate}
	...
	\item Dar formato al volumen, copiar archivos, modificar \textit{fstab} y liberar espacio (igual que en la sesión anterior)
\end{enumerate}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[arrow/.style = {thick,-stealth}, node distance=2cm]
		...
		% Host
		\node[diamond,fit={(0,0) (1,1)}, inner sep=0pt, draw=black, thick, text centered,fill=violet!20] (host) {Host};

		% NAT
		\node[rectangle, above left of =host,node distance = 3cm,inner sep=0pt, draw=black, thick, text centered,fill=red!50, minimum size = 1cm] (nat1) {NAT};
		\node[rectangle, above of =nat1,node distance = 3cm,inner sep=0pt, draw=black, thick, text centered,fill=red!50, minimum size = 1cm] (nat2) {NAT};

		\node[rectangle, above right of =host,node distance = 3cm,inner sep=0pt, draw=black, thick, text centered,fill=red!50, minimum size = 1cm] (nat3) {NAT};
		\node[rectangle, above of =nat3,node distance = 3cm,inner sep=0pt, draw=black, thick, text centered,fill=red!50, minimum size = 1cm] (nat4) {NAT};


		% CentOS

		\node[circle, right of=nat4,node distance = 3cm, minimum size = 2cm,inner sep=0pt, draw=black, thick, text centered,fill=blue!30] (centos) {CentOS};

		% Ubuntu Server
		\node[circle, left of=nat2,node distance = 3cm, minimum size = 2cm,inner sep=0pt, draw=black, thick, text centered,fill=yellow!20, align=center] (ubuntu) {Ubuntu \\ Server};

		% Internet
		\node[cloud, right of=nat3,node distance = 3cm, minimum width=3cm,inner sep=0pt, draw=black, thick, text centered,fill=blue!10, align=center] (internet) {Internet};


		%% Host- only
		\node[rectangle, above right of =ubuntu,node distance = 3cm,inner sep=0pt, draw=black, thick, text centered,fill=green!50, minimum size = 1cm] (nat5) {NAT};
		\node[rectangle, above left of =centos,node distance = 3cm,inner sep=0pt, draw=black, thick, text centered,fill=green!50, minimum size = 1cm] (nat6) {NAT};

		% Arrows

		\draw [arrow] (host) -- (nat1);
		\draw [arrow] (host) -- (nat3);
		\draw [arrow] (nat1) -- (nat2);
		\draw [arrow] (nat3) -- (nat4);

		\draw [arrow] (nat2) -- (ubuntu);
		\draw [arrow] (nat4) -- (centos);

		\draw [arrow] (nat3) -- (internet);

		\draw [arrow,draw=red] (ubuntu) -- (nat5);
		\draw [arrow,draw=red] (nat5) -- (nat6) node[midway,above] (ho) {Host-only};
		\draw [arrow,draw=red] (nat6) -- (centos);

		% IP addresses
		\node[above left of =ubuntu] {192.168.56.105};
		\node[above right of =centos] {192.168.56.115};
		\node[below of =host, node distance = 1.5cm] {192.168.56.1};
